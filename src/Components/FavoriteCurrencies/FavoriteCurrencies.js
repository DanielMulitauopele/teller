    ...
    return (
      <div className="fave-currency">
        ...
        <div className="price-progress">
          <p>
            {favorite.percent_change < 0 ? "-": "+"}${(favorite.percent_change < 0 ? -1 : 1) * Math.round(favorite.price * favorite.percent_change)/100} ({favorite.percent_change}% <img className="arrow" src={Green} />)
          </p>
        </div>
      </div>
    );
